...
def pathSegmentPoints(start: Vec, end: Vec): Set[Vec] =
  iterate(start)(_ + (end - start).sgn).takeWhile(_ != end).toSet + end
...
val (part1, _, _) = iterate((-1, allPathPoints, Dummy))((i, occupied, _) =>
...
val (part2, _, _) = iterate((0, allPathPoints, Dummy))((i, occupied, _) =>
